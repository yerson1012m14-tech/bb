...
struct KeyActivationView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Ingresa tu Key de prueba para continuar")
                    .font(.system(size: 12))
                    .foregroundColor(.gray)
            ...
            VStack(spacing: 12) {
                HStack {
                    ...
                    TextField("JX-DEMO-2026", text: $keyInput)
    ...
    private func formatKeyInput(_ val: String) {
        let clean = val.uppercased().filter { $0.isLetter || $0.isNumber || $0 == "-" }
        self.keyInput = String(clean.prefix(32))
        self.errorMessage = ""
    }
    
    private func validateAndActivateKey() {
        if appState.activateKey(keyInput) {
            isSuccess = true
            errorMessage = ""
            ...
            isSuccess = false
            errorMessage = "⚠ Key de demostración inválida"
