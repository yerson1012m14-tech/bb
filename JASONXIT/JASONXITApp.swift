...
class JASONXITAppState: ObservableObject {
    ...
    @Published var licenseKey: String = "JX-DEMO-2026"
    static let demoKey = "JX-DEMO-2026"
    ...
    init() {
        isLicenseActive = UserDefaults.standard.bool(forKey: "JASONXIT.demoKeyActive")
        refreshSystemInfo()
    ...
    func generateSampleKey() {
        self.licenseKey = Self.demoKey
    }
    
    @discardableResult
    func activateKey(_ key: String) -> Bool {
        let normalized = key.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        guard normalized == Self.demoKey else {
            isLicenseActive = false
            addLog("Key de demostración inválida.", level: .warn)
            return false
        }
        self.licenseKey = Self.demoKey
        self.isLicenseActive = true
        UserDefaults.standard.set(true, forKey: "JASONXIT.demoKeyActive")
        addLog("Key de demostración activada correctamente.", level: .success)
        return true
    }
